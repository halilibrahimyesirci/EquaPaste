...
Hover any equation in ChatGPT, Claude or Gemini — the whole block highlights and a Copy button
...
Works today on ChatGPT, Claude, and Gemini. Copilot and Perplexity are planned.
...
- **storage** — Stores the user's preferences (default target, theme) locally. No data
  leaves the device.
- **Host permissions (chatgpt.com, claude.ai, gemini.google.com)** — Limited to the AI
  chat domains where rendered equations appear, so the content script can detect and
  convert them. No broad `<all_urls>` access is requested.
